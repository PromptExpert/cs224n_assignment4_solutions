...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        #print (P.shape) # torch.Size([55, 32, 39784])
        # Zero out, probabilities for which we have nothing in the target text
        target_masks = (target_padded != self.vocab.tgt['<pad>']).float()
        # print (target_masks.shape) #torch.Size([59, 32])
        
        # Compute log probability of generating true target words
        # print (target_padded[1:].unsqueeze(-1).shape) #torch.Size([55, 32, 1])
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens = enc_hiddens.permute(1,0,2) #torch.Size([5, 22, 6])
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        #print (src_encodings.shape) # (1,maxlen,512)
        src_encodings_att_linear = self.att_projection(src_encodings)

        h_tm1 = dec_init_vec
        att_tm1 = torch.zeros(1, self.hidden_size, device=self.device) # 其实是o_t,combined-output vector
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            # exp_src_encodings,src_encodings沿着第0维复制hyp_num份，其他维度保持不变
            ...
            # print (att_t.shape) #(5,256)
            # 有5个attention向量，每个映射到一个词表的概率分布
            log_p_t = F.log_softmax(self.target_vocab_projection(att_t), dim=-1)
            # print (log_p_t.shape) #torch.Size([5, 39784])

            live_hyp_num = beam_size - len(completed_hypotheses) # 活跃的假设数量，即尚未完成的假设数量
            # print (hyp_scores.shape) # torch.Size([5])，一个向量的分数，
            # hyp_scores.unsqueeze(1).expand_as(log_p_t)是把每个假设的累计分数扩充词表大小份，得到(5,39784)的张量
            # 加上log_p_t后，变成新的累加分数，某行的第i个元素表示相应假设加上第i个词之后的分数
            # 最后view(-1)将矩阵拉平，放到一行
            contiuating_hyp_scores = (hyp_scores.unsqueeze(1).expand_as(log_p_t) + log_p_t).view(-1)
            # print (contiuating_hyp_scores.shape) # torch.Size([198920]), 39784*5
            top_cand_hyp_scores, top_cand_hyp_pos = torch.topk(contiuating_hyp_scores, k=live_hyp_num)

            # 由于contiuating_hyp_scores是拉平的，所以每隔39784个分数就是新的假设
            # top_cand_hyp_pos除以39784就是假设的序号
            prev_hyp_ids = top_cand_hyp_pos / len(self.vocab.tgt) 
            # top_cand_hyp_pos除余39784就是新的预测的词的id
            hyp_word_ids = top_cand_hyp_pos % len(self.vocab.tgt) 


            new_hypotheses = []
            live_hyp_ids = [] # 用于记录下一轮需要保留的隐状态
            new_hyp_scores = []

            # 对每一个最新假设（路径）
            # prev_hyp_id标记了路径
            # hyp_word_id标记了最新的词
            # cand_new_hyp_score标记了分数
            ...
            att_tm1 = att_t[live_hyp_ids] #其实是o_t,combined-output vector

            hypotheses = new_hypotheses
            hyp_scores = torch.tensor(new_hyp_scores, dtype=torch.float, device=self.device)

        # 如果最大解码次数达到了，但还是没有eos的路径，那么就凑合用当前的路径
